...
function showVictory() {
  ...
  // Mostrar anuncio antes de mostrar el modal si existe
  if (window.Android && typeof Android.showInterstitial === "function") {
    Android.showInterstitial();
    setTimeout(() => {
      renderVictoryModal();
    }, 100); // puedes aumentar si necesitas más tiempo
  } else {
    renderVictoryModal();
  }
}
...
function renderVictoryModal() {
  ...
  menuButton.onclick = () => {
    // Mostrar anuncio antes de ir al menú, si está disponible
    if (window.Android && typeof Android.showInterstitial === "function") {
      Android.showInterstitial();
      setTimeout(() => {
        victoryMusic.pause();
      ...
      }, 100); // Puedes aumentar a 1000ms si el anuncio necesita más tiempo
    } else {
      victoryMusic.pause();
      victoryMusic.currentTime = 0;
      window.location.href = "index.html";
